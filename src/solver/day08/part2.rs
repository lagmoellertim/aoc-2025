...
fn solve_day08_part02_puzzle(input: &str) -> String {
    ...
    let mut coordinates_in_clusters = HashSet::new();
    ...
    for (_, i, j) in connections {
        ...
        coordinates_in_clusters.insert(i);
        coordinates_in_clusters.insert(j);
        ...
        match (cluster_i, cluster_j) {
            ...
            (_, Some(cluster_j)) => {
                clusters[cluster_j].insert(j);
            }
            (_, _) => clusters.push(HashSet::from([i, j])),
        }

        if clusters.len() == 1 && coordinates_in_clusters.len() == coordinates.len() {
            result = Some(coordinates[i].x * coordinates[j].x);
            break;
        }
    }
...
register_solver! {
    day: 8,
    part: 2,
    solver_fn: solve_day08_part02_puzzle
}
